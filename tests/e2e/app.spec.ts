...
test("complete deterministic sightline, sunrise, export and responsive workflow", async ({
  page,
  context,
}) => {
  const errors: string[] = [];
  ...
  await context.route(
    "https://geoservices.bayern.de/services/poly2metalink/zip/start/**",
    async (route) => {
      const body = route.request().postData()!,
        match = body.match(/\(\((\d+) (\d+)/)!;
      const id = `${Math.floor(Number(match[1]) / 1000)}_${Math.floor(Number(match[2]) / 1000)}`;
      await route.fulfill({
        json: {
          status: "FINISHED_OK",
          url: `https://geodaten.bayern.de/odd_zip/TEST_FIXTURE/${id}.zip`,
        },
      });
    },
  );
  await context.route(
    "https://geodaten.bayern.de/odd_zip/TEST_FIXTURE/**",
    async (route) => {
      const id = route.request().url().split("/").at(-1)!.replace(".zip", "");
      await route.fulfill({
        body: Buffer.from(
          zipSync({ [`${id}.tif`]: new Uint8Array(fixtureTIFF(id)) }),
        ),
  ...
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "Sichtlinie & Horizont" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Gelände in der Sichtlinie" }),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "Analyse starten" }).click();
  await expect(
    page.getByRole("heading", { name: "Horizontfunktion H(A)" }),
  ).toBeVisible();
  ...
  await page.screenshot({
    path: `_local_audit/screenshots/e2e-${test.info().project.name}.png`,
    fullPage: true,
  });
});
test("invalid input and cancellation have clear states", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("Breite ° N", { exact: true }).fill("0");
  await page.getByRole("button", { name: "Analyse starten" }).click();
