...
export function tileId(x: number, y: number) {
  return `${Math.floor(x / 1000)}_${Math.floor(y / 1000)}`;
}
export function tileOrigin(id: string) {
  if (!/^\d{3}_\d{4}$/.test(id)) throw new Error("Ungültige Kachel-ID");
  return id
    .split("_")
    .map(Number)
    .map((x) => x * 1000) as [number, number];
}
...
export function normalRadius(latitude: number, azimuth: number) {
  const a = 6378137,
    f = 1 / 298.257222101,
    e2 = f * (2 - f),
    q = 1 - e2 * Math.sin(latitude * RAD) ** 2;
  const N = a / Math.sqrt(q),
    M = (a * (1 - e2)) / q ** 1.5;
  return (
    1 / (Math.cos(azimuth * RAD) ** 2 / M + Math.sin(azimuth * RAD) ** 2 / N)
  );
}
export function curvatureDrop(distance: number, radius: number, k = 0) {
  return ((1 - k) * distance * distance) / (2 * radius);
}
export function apparentAngle(
  distance: number,
  height: number,
  observer: number,
  radius: number,
  ...
  if (distance <= 0 || !Number.isFinite(distance))
    throw new Error("Entfernung muss positiv sein");
...
export function corridorTiles(
  ...
  if (end < start || end - start > 180 || distance <= 0 || distance > 80000)
    throw new Error("Ungültiger Korridor");
  const ids = new Set<string>(),
    count = Math.max(1, Math.ceil(((end - start) * RAD * distance) / 250));
  for (let a = 0; a <= count; a++)
    for (let d = 0; d <= Math.ceil(distance / 250); d++) {
      const [x, y] = toUTM(
        destination(
          observer,
          start + ((end - start) * a) / count,
          Math.min(distance, d * 250),
        ),
      );
      for (const dx of [-1000, 0, 1000])
        for (const dy of [-1000, 0, 1000]) ids.add(tileId(x + dx, y + dy));
    }
  return [...ids].sort();
}
